...
The separate all-`n` Poisson research program now has an exact
...
the latter is now proved for every profile with at most three nonzero
coefficients, on a sparse convex-core region of every coordinate face, and
on the four-positive far cap `d>=n-1`, and completely for `n=2`, `n=3`, and
`n=4`.  At `n=5`, every profile with at most four nonzero coordinates is
proved, leaving only the five-positive boundary.  The first open complete
simplex dimension is `n=5`.
The program note
...
point; it is also proved on the equal-smaller symmetry line, where the
...
A fixed-total-gap comparison and a direct tail bound prove the full triangle
in which the two gaps sum to at most `8/9`, and an analytic Gram-matrix
comparison proves the full region in
which both gaps are at least `13`.  Its remaining finite off-symmetry region,
whose total gap exceeds `8/9` and whose smaller gap is below `13`, remains
open.
...
| `theory/THREE-EXPONENTIAL-QUANTILE.md` | Exact tilted-simplex Hessian, boundary-trace, and tail reduction of three-weight convexity, with proofs on the repeated-maximum and infinite-gap boundaries, the equal-smaller symmetry line, strict fixed-sum derivative signs at both finite symmetry boundaries, a sharp-corner neighborhood, the total-gap-at-most-8/9 triangle, and the region with both gaps at least 13; the remaining off-symmetry region outside those sets is explicit | proof-essential for the reduction, boundary, symmetry-line, local derivative, local-corner, small-total-gap, and two-large-gap theorems; research roadmap for the remaining interior |
| `theory/DIRICHLET-POISSONIZATION.md` | Constrained divided-difference reduction, zero-knot boundary reduction, proofs for every two-level profile, every profile with at most three nonzero coefficients, a sparse convex core on every coordinate face, the four-positive far cap, the complete `n=2`, `n=3`, and `n=4` cases, and the complete four-positive face at `n=5`, plus an exact obstruction to generic `s`-concave localization | proof-essential for the structured-family, sparse-core, four-positive-far-cap, full three-positive-face, `n=2`, `n=3`, computer-assisted `n=4`, and computer-assisted `n=5` four-positive-face theorems; research roadmap in general |
...
| `dirichlet_poissonization.py` | Symbolic, exact-rational, and directed real-ball checks for the zero-knot, all-two-level, all-`n` two-positive-knot, arbitrary-order sparse-core, four-positive-far-cap, complete all-`n` three-positive-face, complete `n=2`, `n=3`, and `n=4` reductions, and the complete `n=5` four-positive face, plus the exact `s`-affine obstruction; does not claim the general inequality | proof-essential for the finite-prefix component of the three-positive-face theorem, both exhaustive interval components of the `n=4` theorem, and the `n=5` four-positive-face certificate; proof support for the other checked identities; research support in general |
