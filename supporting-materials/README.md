...
on the four-positive far cap `d>=n-1`, and completely for `n=2` and `n=3`.
The program note
...
| `dirichlet_poissonization.py` | Symbolic, exact-rational, and rigorous real-ball checks for the zero-knot, all-two-level, all-`n` two-positive-knot, arbitrary-order sparse-core, four-positive-far-cap, complete all-`n` three-positive-face, complete `n=2`, and complete `n=3` reductions plus the exact `s`-affine obstruction; does not claim the general inequality | proof-essential for the finite-prefix component of the three-positive-face theorem and proof support for the other checked identities; research support in general |
