...
on the four-positive far cap `d>=n-1`, and completely for `n=2`, `n=3`,
`n=4`, `n=5`, and `n=6`. The first open complete simplex dimension is `n=7`.
The program note
...
| `dirichlet_poissonization.py` / `n6_dirichlet_poissonization.py` | Symbolic, exact-rational, and directed real-ball checks for the zero-knot, all-two-level, all-`n` two-positive-knot, arbitrary-order sparse-core, four-positive-far-cap, complete all-`n` three-positive-face, and complete `n=2` through `n=6` reductions, plus the exact `s`-affine obstruction; does not claim the general inequality | proof-essential for the finite-prefix component of the three-positive-face theorem and the exhaustive interval components of the `n=4`, `n=5`, and `n=6` theorems; proof support for the other checked identities; research support in general |
...
# Dirichlet--Poissonization reductions, including the complete n=6 theorem
make dirichlet-poissonization-check
# Optional: set STRINGER_N6_WORKERS to control parallel root regeneration
