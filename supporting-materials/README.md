...
the latter is now proved for every profile with at most two nonzero
coefficients, for dimension-free convex-core and far-cap regions and the full
middle-knot region `b<=n/3` with three nonzero coefficients, and completely
for `n=2` and `n=3`.
The program note
...
| `theory/DIRICHLET-POISSONIZATION.md` | Constrained divided-difference reduction, zero-knot boundary reduction, proofs for every two-level profile, every profile with at most two nonzero coefficients, dimension-free three-positive convex-core and far-cap regions and the middle-knot region `b<=n/3`, and the complete `n=2` and `n=3` cases, plus an exact obstruction to generic `s`-concave localization | proof-essential for the structured-family, convex-core, far-cap, middle-knot, `n=2`, and `n=3` theorems; research roadmap in general |
...
| `dirichlet_poissonization.py` | Symbolic and exact-rational checks for the zero-knot, all-two-level, all-`n` two-positive-knot, all-`n` three-positive convex-core and far-cap regions and the middle-knot region `b<=n/3`, complete `n=2`, and complete `n=3` reductions plus the exact `s`-affine obstruction; does not claim the general inequality | proof support for the checked identities; research support in general |
