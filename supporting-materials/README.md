...
coefficients and completely for `n=2`.  The program note
...
| `dirichlet_poissonization.py` | Symbolic checks for the zero-knot, all-two-level, all-`n` two-positive-knot, and complete `n=2` reductions plus the exact `s`-affine obstruction; does not claim the general inequality | proof support for the checked identities; research support in general |
