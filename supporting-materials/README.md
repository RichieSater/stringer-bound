...
least \(1-e^{-2}\approx86.5\%\): the safeguard is ordinary Stringer whenever
...
| `theory/ONE-CAP-COMPARISON.md` | Dimension-free cap lemma and analytic all-sample-size Stringer specialization at confidence at least \(1-e^{-2}\) | proof-essential |
...
# Analytic all-n one-cap theorem at confidence >=1-exp(-2)
...
\(1-e^{-2}\approx86.5\%\) and every sample size, the additional one-cap
