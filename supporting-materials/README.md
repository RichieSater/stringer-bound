...
| `dirichlet_poissonization.py` | Symbolic and exact-rational checks for the zero-knot, all-two-level, all-`n` two-positive-knot, complete `n=2`, and complete `n=3` reductions plus the exact `s`-affine obstruction; does not claim the general inequality | proof support for the checked identities; research support in general |
