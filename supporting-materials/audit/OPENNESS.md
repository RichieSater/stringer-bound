...
\(n=3\) for 90%, 95%, and 99% confidence. Thus \(n=4\) is the first
unresolved sample size at those three levels. This does not resolve every
confidence level at \(n=3\), nor arbitrary \(n\). No prior general proof or
...
| Binomial Stringer is distribution-free conservative at \(n=2\) for every α, sharply; at \(n=3\), it pointwise dominates the valid Gaffke bounded-mean upper limit at α = 0.10, 0.05, and 0.01 | this repository: [`N2-PROOF.md`](../theory/N2-PROOF.md), [`N3-CONVENTIONAL.md`](../theory/N3-CONVENTIONAL.md), and exact rational certificates | finite n; all levels at \(n=2\), 90%, 95%, and 99% at \(n=3\) |
...
`theory/N3-CONVENTIONAL.md`, and `computations/certificates/`): a
distribution-free proof at \(n=2\); an exact computer-assisted proof at
\(n=3\) for α = 0.10, 0.05, and 0.01; and 33 exact rational
...
- **Gaffke's bounded-mean interval** is now known to be finite-sample valid
  ...
  prove the three stated \(n=3\) cases. No corresponding all-\(n\)
...
5. Extension of the Gaffke/simplex-cap comparison to \(n=4\), the first
