...
def fail_prob(atoms, probs, A, B, w):
    p = 0.0
...
def stress(alphas=(0.05, 0.1, 0.5, 0.7, 0.9), trials: int = 40000) -> float:
    ...
    for a in alphas:
        ...
        for trial in range(trials):
            ...
            if w <= 1e-9:
                continue
            worst = max(worst, fail_prob(atoms, pr, A, B, w) - a)
        for q in np.linspace(0.001, 0.999, 400):
            for u in np.linspace(0.01, 1.0, 50):
                worst = max(worst,
                            fail_prob([0.0, u], [1 - q, q], A, B, q * u) - a)
    return worst
...
def main():
    ...
    ok &= r
    excess = stress()
    print("theorem stress test: max[P(fail) - alpha] over atomic F = %+.2e %s"
