...
The off-symmetry strict interior remains open.
...
1. Prove the explicit tilted-simplex inequality in the off-symmetry strict
   interior `z,w>0`, `z!=w`, in
   ...
   repeated-maximum boundary `zw=0` and equal-smaller diagonal `z=w` are
   already proved.  Then seek a dimension-free argument for (11).
